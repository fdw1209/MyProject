...
public class CompDev1TestTable extends AbstractComposite {
		...
		btnStart.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				if (view.isDisposed() && CompDev1NICConfView.isConfirm) {
					ManagerPanel.getInstance().setStatus("设备1WiFi吞吐量开始测试", SWT.COLOR_DARK_GREEN, false);
					lbl_result.setText("");
	...
	public void consultTotalData() {
		...
		Display.getDefault().asyncExec(new Runnable() {
			...
			public void run() {
				updateData();
				ManagerPanel.getInstance().setStatus("设备1WIFI吞吐量测试结束", SWT.COLOR_DARK_GREEN, false);
				...
				try {
					if (wifiDatas.get(6).averageRate != 0 && wifiDatas.get(7).averageRate != 0) {
						String netType = CompDev1NICConfView.getProp().getProperty("Dev1_NetType");
						XmlParse.saveDataForXML(CompDev1NICConfView.getProp().getProperty("Dev1_Sn"), netType,
					...
					ManagerPanel.getInstance().setStatus("设备1测试报告成功导出", SWT.COLOR_DARK_GREEN, false);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
