...
public class ReceiveThread extends Thread {
	...
	public ReceiveThread(WifiData wifiData, String devName) {
		this.wifiData = wifiData;
	...
	public void run() {
		try {
			...
			if (devName.equals("Dev1")) {
				socket.bind(new InetSocketAddress(CompDev1NICConfView.getProp().getProperty("Dev1_CableIp"), 0));
				socket.connect(new InetSocketAddress(CompDev1NICConfView.getProp().getProperty("Dev1_WirelessIp"), 8877), 5000);
			}else {
				socket.bind(new InetSocketAddress(CompDev2NICConfView.getProp().getProperty("Dev2_CableIp"), 0));
				socket.connect(new InetSocketAddress(CompDev2NICConfView.getProp().getProperty("Dev2_WirelessIp"), 9988), 5000);
			}
