...
import com.ch.sockettest.popview.CompDev1NICConfView;
import com.ch.sockettest.popview.CompDev2NICConfView;
...
public class PostThread extends Thread {
	...
	public PostThread(WifiData wifiData, String devName) {
		this.dataBuffer = wifiData;
	...
	public void run() {
		try {
			...
			if (devName.equals("Dev1")) {
				socket.bind(new InetSocketAddress(CompDev1NICConfView.getProp().getProperty("Dev1_WirelessIp"), 0));
				socket.connect(new InetSocketAddress(CompDev1NICConfView.getProp().getProperty("Dev1_CableIp"), 7788), 5000);
			}else {
				socket.bind(new InetSocketAddress(CompDev2NICConfView.getProp().getProperty("Dev2_WirelessIp"), 0));
				socket.connect(new InetSocketAddress(CompDev2NICConfView.getProp().getProperty("Dev2_CableIp"), 8899), 5000);
			}
